...
const refreshUnits = async (degree : 'C' | 'F') =>{
    units = degree == 'C' ? 'metric' : 'imperial';
}
...
const formatParams = (latitude : number, longitude : number ) =>{
    return `lat=${latitude}&lon=${longitude}&appid=${openWeatherKey}&lang=pt_br&units=${units}`
}

const formatOneCallParams = (latitude : number, longitude : number ) =>{
    return `lat=${latitude}&lon=${longitude}&exclude=minutely,hourly,alerts,current&appid=${openWeatherKey}&lang=pt_br&units=${units}`
}
